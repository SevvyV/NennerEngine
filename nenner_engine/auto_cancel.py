...
def check_auto_cancellations(
    ...
    # Atomic block: every auto-cancel INSERT plus the state rebuild commit
    # together, or roll back together. Without this, a crash after the
    # commit but before the rebuild leaves CANCELLED rows in `signals`
    # without a corresponding implied-reversal entry in current_state —
    # the watchlist would still display the dead BUY/SELL until the next
    # rebuild trigger ran.
    if conn.in_transaction:
        log.error(
            "check_auto_cancellations called inside an existing transaction — refusing"
        )
        return []
    conn.execute("BEGIN")
    try:
        ...
        if cancellations:
            ...
            # its own with-block, so the rebuild is part of THIS commit.
