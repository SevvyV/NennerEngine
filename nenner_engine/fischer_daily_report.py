...
def _build_recommendation_email(
    ...
    conviction_legend = (
        '<strong>Conviction</strong> = Nenner signal conviction score (0&ndash;100)<br>'
        if show_conviction else ""
    )
    ...
    return _wrap_document(
        ...
        footer_text="Generated by Fischer Options Engine | Vartanian Capital Management",
        notes_html=legend,
        max_width=960,
    )
...
def _build_unified_email(
    ...
    """
    today = date.today().strftime("%B %d, %Y")
    header = _table_header(show_conviction=True)
    ...
    for section in REPORT_SECTIONS:
        ...
        rows_html = "".join(_build_rec_row(r, show_conviction=True) for r in sorted_recs)
...
def _send_offline_alert(send_email, config: ScanSlotConfig):
    ...
    subject = f"\u26a0 Fischer Options \u2014 No Data \u2014 {today_fmt}"
    body = _wrap_document(
        body_html=f"""
...
def send_scan_report(db_path: str, slot: ScanSlot = "opening"):
    ...
    if rel and rel.cache:
        ...
        if cached:
            ...
            today_fmt = date.today().strftime('%b %d')
            subject = f"Fischer Options \u2014 {config.label} \u2014 {today_fmt}"
